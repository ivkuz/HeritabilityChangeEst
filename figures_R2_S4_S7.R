...
###############################################################################
# Plot Fig. S4 and S7, Untransmitted, sex-specific, ###########################
# cognitive/non-cognitive, settlement type ####################################
...
for(sex in c("male", "female")){
  ...
  r2_res_v <- paste0("r2_adj_", c("EduYears", "OS", "Height_first", "BMI_first"), "_", sex, ".tsv")
  bootstrap_v <- paste0("r2_adj_", c("EduYears", "OS", "Height_first", "BMI_first"), "_", sex, "_1000.tsv")
  titles <- c("EA", "OS", "Height", "BMI")
...
for(settlement in c("rural", "town", "city")){
  ...
  r2_res_v <- paste0("r2_adj_", c("EduYears", "OS", "Height_first", "BMI_first"), "_", settlement, ".tsv")
  bootstrap_v <- paste0("r2_adj_", c("EduYears", "OS", "Height_first", "BMI_first"), "_", settlement, "_1000.tsv")
  titles <- c("EA", "OS", "Height", "BMI")
  lim_list_inc <- c(0.17, 0.14, 0.25, 0.17)
  steps_inc = c(0.05, 0.05, 0.1, 0.05)
  lim_list <- c(0.18, 0.14, 0.5, 0.19)
  steps = c(0.05, 0.05, 0.1, 0.05)
...
lim_list_inc <- c(0.05, 0.05, 0.05, 0.05)
steps_inc = c(0.02, 0.02, 0.02, 0.02)
lim_list <- c(0.05, 0.05, 0.05, 0.05)
steps = c(0.02, 0.02, 0.02, 0.02)
...
# Cognitive/noncognitive: untransmitted
r2_res_v <- paste0("untransmitted_r2_adj_", rep(c("EduYears", "OS"), each = 2), "_", rep(c("Cog", "Noncog"), 2), ".tsv")
bootstrap_v <- paste0("untransmitted_r2_adj_", rep(c("EduYears", "OS"), each = 2), "_", rep(c("Cog", "Noncog"), 2), "_1000.tsv")
titles <- c("EA - Cog", "EA - NonCog", "OS - Cog", "OS - NonCog")
lim_list_inc <- c(0.07, 0.05, 0.05, 0.05)
steps_inc = c(0.02, 0.02, 0.02, 0.02)
lim_list <- c(0.1, 0.08, 0.1, 0.05)
steps = c(0.02, 0.02, 0.02, 0.02)
