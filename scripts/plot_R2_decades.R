...
r2Decades <- function(ebb_test, trait, age, prs, title){
  ...
  for(k in c(5,10)){
    ...
    for(i in seq(1930, 1990, k)){
      ...
      ebb_test_bin <- ebb_test[YoB > i & YoB <= i+k, ]
    ...
    pl2 <- ggplot(r2_decades[N>=100 & phase != "all",], aes(x=Birth, y=R2, group=phase, color=phase)) + 
      ...
      theme(text = element_text(size = 10),
            ...
            axis.title.x=element_blank(),
            legend.position = "none") +
      scale_color_manual(values=c("#ad8d49", "#38785e"))
...
pl3 <- r2Decades(ebb_test = ebb_test, trait = "EduYears", age = "Age", prs = "PRS", title = "EA")
pl4 <- r2Decades(ebb_test = ebb_test, trait = "OS", age = "Age", prs = "PRS", title = "OS")
...
for(i in 1:4){
  ...
  plot_list <- list(pl3[[i]], pl4[[i]], pl1[[i]], pl2[[i]])
